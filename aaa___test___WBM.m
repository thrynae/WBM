function aaa___test___WBM(varargin)
...
if ~RunTestHeadless
    %Save the breakpoints, but clear everything else, so the function can be
    %evaluated without anything pre-loaded. This should work for >R2006b
    try
        s=dbstatus('-completenames');
        current_folder=[fileparts(which(mfilename)) filesep];
        save([current_folder 's.mat'],'s')
    catch
    end
    clear all %#ok<CLALL>
    RunTestHeadless=false;%restore variable
    try
        current_folder=[fileparts(which(mfilename)) filesep];
        s=load([current_folder 's.mat']);s=s.s;
        delete([current_folder 's.mat'])
        dbstop(s)
    catch
        %probably an old enough release that this is needed
        warning off MATLAB:m_warning_end_without_block
    end
end

global websavepause pausetime
websavepause=0.1;pausetime=0;%keep track of total pause time

clc
starttime=now;

%% write test files and shadow function
url='example.com';
date='20170715213747';
%use a global to force websave to trigger a specific error sometimes
global trigger_websave_error
trigger_websave_error=[];
...
try
    %run the tests
    f=[];
    %% trigger a compile attempt of utc_time.c
    try
        WBM
    catch
        ME=lasterror;%#ok<LERR>
        if ~strcmp(ME.identifier,'HJW:WBM:nargin')
            %compile failed and no internet available for the web fallback method (or the UTC
            %offset is larger than 14 hours, wich shouldn't be possible)
            disp(ME.identifier)
            error('test failed')
        end
    end
    if ~RunTestHeadless
        f(end+1) = msgbox(sprintf('Test %d Completed',numel(f)+1));
        pause(0.01),drawnow
    ...
    options=struct;
    options.err429=struct('CountsAsTry',true,'TimeToWait',0.5, 'PrintAtVerbosityLevel',3);
    then=now;tic
    WBM(file01,url,options),toc
    time_elapsed=(now-then)*60*60*24;
    pausetime_this_cell=2*websavepause+options.err429.TimeToWait;
    pausetime=pausetime+pausetime_this_cell;%running total
    if time_elapsed<pausetime_this_cell
        error('test failed (%.1f<%.1f=%.5f)',time_elapsed,pausetime,(time_elapsed-pausetime))
    end
    if ~RunTestHeadless
        f(end+1) = msgbox(sprintf('Test %d Completed',numel(f)+1));
        pause(0.01),drawnow
    ...
    id_FormatSpec='MATLAB:webservices:HTTP%dStatusCodeError';
    ME=struct;
    ME(1).message='foo';ME(1).identifier=sprintf(id_FormatSpec,403);
    ...
    %occurred.
    warning('HJW:tester:BlankWarning','NoWarningOccured');clc
    out=WBM([current_folder 'doesnt_exist.txt'],url);
    ...
    %% successful read from date
    out=WBM(file02,url,struct('date_part',date(1:8)));
    if ~isequal(out,file02) || ~strcmp(ME.identifier,'HJW:tester:BlankWarning')
        error('test failed')
    end
    if ~RunTestHeadless
        f(end+1) = msgbox(sprintf('Test %d Completed',numel(f)+1));
    ...
    out=WBM(file01,url,'date_part','2000');
    if ~isempty(out) || exist(file01,'file') || ~strcmp(ME.identifier,'HJW:tester:BlankWarning')
        error('test failed')
    end
    if ~RunTestHeadless
        f(end+1) = msgbox(sprintf('Test %d Completed',numel(f)+1));
    ...
    out=WBM(file03,url,'date_part','2000');
    ME=struct;[ME.message,ME.identifier] = lastwarn;
    if ~isequal(out,file03) || ~strcmp(ME.identifier,'HJW:WBM:MissingDateWarning')
        error('test failed')
    else
        clc
    end
    ...
    %occurred.
    warning('HJW:tester:BlankWarning','NoWarningOccured');clc
    out=WBM([current_folder 'doesnt_exist.txt'],url,'UseURLwrite',true);
    ME=struct;[ME.message,ME.identifier] = lastwarn;
    if ~isempty(out) || ~strcmp(ME.identifier,'HJW:tester:BlankWarning')
        if ~strcmp(ME.identifier,'MATLAB:mir_warning_unrecognized_pragma')
            error('test failed')
        else
            error('test unreliable when not compiled')
        end
    else
        clc
    end
    ...
    all_tests_OK=false;
    ME2=lasterror;%#ok<LERR>
end
%% delete test files and shadow function
if exist(file01,'file'),delete(file01);end
if exist(file02,'file'),delete(file02);end
if exist(file03,'file'),delete(file03);end
if exist([current_folder 'weboptions.m'],'file')
    delete([current_folder 'weboptions.m']);
end
%% rethow on test fail
...
if ~RunTestHeadless
    try uiwait(f(end));catch,end
    for n=1:numel(f),try delete(f(n));catch,end,end
end
end
